...
spec =
       describe "strdsl" $ do
              it "left" $ do 
                     synth ([([Vs "hello"], Vs "h"), ([Vs "world"], Vs "w")], ["x"], StrDsl.Placeholder, StrDsl.ConcatOp, StrDsl.Es) `shouldBe` S (LeftS (X (Input "x")) (Const 1))
              it "right" $ do 
                     synth ([([Vs "hello"], Vs "o"), ([Vs "world"], Vs "d")], ["x"], StrDsl.Placeholder, StrDsl.ConcatOp, StrDsl.Es) `shouldBe` S (RightS (X (Input "x")) (Const 1))
              it "concat" $ do 
